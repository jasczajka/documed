...
public class ReferralDAO implements FullDAO<Referral, CreateReferralDTO> {
    ...
    public Referral create(CreateReferralDTO createReferralDTO) {
        String sql = "INSERT INTO referral (diagnosis, type, expiration_date, visit_id) VALUES (?, ?, ?, ?) RETURNING id";
        ...
        jdbcTemplate.update(
                connection -> {
                    ...
                    ps.setInt(1, createReferralDTO.getVisitId());
        ...
        if (key != null) {
            return Referral.builder().id(key.intValue())
            ...
            throw new CreationFailException("Failed create referral");
        }
    }
