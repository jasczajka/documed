...
public class PrescriptionDAO implements FullDAO<Prescription> {
    ...
    public int delete(int id) {
        String deleteMedicinesPrescription = "DELETE FROM medicine_prescription WHERE prescription_id = ?";
    ...
    public Optional<Medicine> addMedicineToPrescription(int prescriptionId, String medicineId, int amount) {
        String sql = "INSERT INTO medicine_prescription (prescription_id, medicine_id, amount) VALUES (?, ?, ?)";
    ...
    public Prescription issuePrescription(int prescriptionId) {
        ...
        if (rowsAffected > 0) {
            return getById(prescriptionId).orElseThrow(() -> new IllegalStateException("Could not found prescription"));
        } else {
